...
fetch('https://fe-apps.herokuapp.com/api/v1/fitlit/1908/hydration/hydrationData')
  ...
  var bar = new ProgressBar.Circle('.number-of-steps-day', {
    ...
    step(state, circle) {
      ...
      if (value === 0) {
        ...
        circle.setText(`${activity.returnUserDataForDay(userData, user.id, date, 'numSteps')} steps`);
      }

    }
  });

  let percentSteps = activity.returnUserDataForDay(userData, user.id, date, 'numSteps') / user.dailyStepGoal;
  ...
  $('.number-of-minutes-active-day').text(`${activity.returnUserDataForDay(userData, user.id,date, 'minutesActive')}`);
  $('.average-minutes-active').text(`${activityRepo.returnAverage(date, 'minutesActive')}`)
  $('.distance').text(`${activity.returnUserDataForDay(userData, user.id, date, 'numSteps')}`);
  $('.average-distance').text(`${activityRepo.returnAverage(date, 'numSteps')}`)
  $('.stairs').text(`${activity.returnUserDataForDay(userData, user.id, date, 'flightsOfStairs')}`);
